...
def test_render_public_stock_page_contains_chart_and_payload(monkeypatch):
    ...
    assert "가격, AI, 리포트, 사후 기록으로 바로 이동합니다" not in html
    ...
    assert 'href="/analyses?ticker=005930">공개 리포트</a>' in html
...
def test_render_public_home_page_is_usable_analysis_explorer():
    ...
    assert "종목을 검색하면 차트, 공시, 뉴스, 공개 리포트, 사후 기록을 한 번에 확인합니다." in html
...
def test_render_member_dashboard_exposes_only_public_supabase_config(monkeypatch):
    ...
    assert "새 일지 묶음" in html
    assert "먼저 매매 기록을 묶을 이름을 만듭니다." in html
    ...
    assert "매매 일지 묶음을 만들었습니다." in html
...
def test_render_feature_detail_pages_use_public_theme():
    ...
    assert "종목을 검색하면 가격·출처·공개 리포트가 한 화면에 모입니다" in html
...
def test_render_admin_console_page_keeps_worker_secret_client_supplied(monkeypatch):
    ...
    assert "실행 전 확인은 저장하지 않습니다." in html
    assert "대기열 처리는 요청 상태를 처리 중으로 바꾸고 공개 리포트를 만듭니다." in html
    assert "사후 기록 계산은 5일/20일 수익률과 벤치마크 차이를 남깁니다." in html
    assert "가상매매 기록은 실제 주문 없이 AI의 가상 진입·청산 기록만 남깁니다." in html
    assert "대상 보는 중" not in html
    assert "실행 전 확인을 누르면 저장 없이 후보를 볼 수 있습니다." in html
...
def test_render_policy_pages_use_public_theme():
    expected = {
        ...
        "disclaimer": ("공개 리포트는 투자 조언이 아니라 검토 자료입니다", "/disclaimer"),
    }
...
def test_render_public_outcomes_page_shows_public_track_record():
    ...
    assert "analysis-feed-metrics" in html
    assert "삼성전자" in html
    assert "사후 기록은 과거 리포트 점검 자료" in html
...
def test_render_public_outcomes_page_empty_state_has_next_actions():
    ...
    assert 'href="/stocks/005930">샘플 종목</a>' in html
    assert 'href="/analyses">공개 리포트</a>' in html
...
def test_render_public_analysis_detail_page_shows_report_context():
    ...
    assert "출처, AI 의견, 본문, 사후 기록을 바로 확인합니다" in html
