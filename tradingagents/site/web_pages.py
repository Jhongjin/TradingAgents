  ...
  <main id="main-content" class="shell market-shell">
    <section class="summary-band" aria-labelledby="stock-title">
      <div>
        ...
        <p class="stock-hero-copy">가격, 공시, 뉴스, 공개 리포트, 사후 기록을 한 화면에서 확인합니다.</p>
    ...
    <section class="workspace">
      ...
      <aside class="side-rail" aria-label="핵심 지표">
        ...
        <section id="stock-simulation-section" class="simulation-panel" aria-labelledby="simulation-title">
          <p class="eyebrow">AI 가상매매 기록</p>
          <h2 id="simulation-title">가상 진입·청산</h2>
  ...
  <main id="main-content" class="home-shell home-shell-art">
    <section class="home-hero home-hero-artboard" aria-labelledby="home-title">
      <div class="home-hero-copy home-hero-content">
        ...
        <p class="home-lede">종목을 검색하면 차트, 공시, 뉴스, 공개 리포트, 사후 기록을 한 번에 확인합니다.</p>
...
FEATURE_DETAIL_PAGES: dict[str, dict[str, Any]] = {
    "research": {
        ...
        "lead": "처음 방문자는 종목명이나 6자리 코드를 검색해 가격 흐름, 데이터 출처, 공개 리포트, 사후 기록을 바로 확인할 수 있습니다. 가입하면 관심그룹을 만들고 필요한 종목의 새 리서치를 요청할 수 있습니다.",
    ...
    "outcomes": {
        ...
        "lead": "공개 리서치는 생성 시점에서 끝나지 않습니다. 사후 기록 작업이 5일/20일 뒤 종목 수익률과 벤치마크 차이를 남겨, 공개 리포트를 이후 흐름과 함께 되돌아볼 수 있게 합니다.",
    ...
    "methodology": {
        ...
        "lead": "공개 리포트는 종목 판단의 근거를 보여주는 자료입니다. KRX, DART, Naver 뉴스, 공개 리포트, 5일/20일 사후 기록을 한 흐름으로 묶되, 투자 실행 권한은 서비스가 갖지 않습니다.",
        ...
        "steps": ("출처 표기", "분석 기준", "공개 리포트", "사후 기록", "주문 없음"),
        "cta_label": "공개 분석 보기",
        "cta_href": "/analyses",
...
<head>
  ...
  <meta name="description" content="TradingAgents Korea의 공개 리서치, 내 공간, 사후 기록, 데이터 기준을 한 번에 확인합니다.">
...
POLICY_PAGES: dict[str, dict[str, Any]] = {
    ...
    "terms": {
        ...
        "lead": "TradingAgents Korea는 한국 주식 공개 데이터, 공개 리포트, 사후 기록, 회원 매매 일지를 제공하는 리서치 플랫폼입니다. 사용자는 정보를 직접 검토하고 판단해야 하며, 서비스는 매매 주문 권한을 갖지 않습니다.",
        ...
        "next_actions": (
            ("공개 분석 보기", "/analyses", "완료된 공개 리포트와 사후 기록 상태를 공개 화면에서 확인합니다."),
            ("가입 후 기록 공간 보기", "/features/member-workspace", "내 관심그룹과 매매 일지가 어떻게 분리되는지 봅니다."),
            ("투자 유의사항 읽기", "/disclaimer", "공개 리포트와 사후 기록을 읽을 때의 한계를 확인합니다."),
        ),
    },
    "disclaimer": {
        "path": "/disclaimer",
    ...
    <section class="admin-grid" aria-label="운영 작업">
      ...
      <article class="admin-card">
        ...
        <div class="button-row">
          ...
          <button type="button" data-admin-action="outcomes-process">사후 기록 계산</button>
        </div>
        <small class="admin-action-help">실행 전 확인은 저장하지 않습니다. 사후 기록 계산은 5일/20일 수익률과 벤치마크 차이를 남깁니다.</small>
      ...
      <article class="admin-card">
        ...
        <div class="admin-action-panel" id="adminPaperSimulationPanel" aria-live="polite">
          <div class="action-cell is-waiting">
            <span>AI 가상매매</span>
            <strong>대기</strong>
            <small>완료 리포트 기준의 가상 진입·청산 근거를 저장합니다. 실제 주문은 없습니다.</small>
          </div>
        </div>
...
def _analysis_track_record_cards(summary: dict[str, Any], *, basis_label: str = "현재 공개 목록 기준") -> str:
    ...
    <section class="analysis-track-record" aria-labelledby="track-record-title">
      <div class="panel-heading">
        <div>
          <p class="eyebrow">사후 기록</p>
          <h2 id="track-record-title">사후 기록</h2>
        </div>
        <span class="status-pill">5일 / 20일</span>
      </div>
  ...
  function renderSimulationState(node, payload) {
    ...
    if (status !== "available") {
      ...
      copy.textContent = status === "no_completed_analysis"
        ? "분석 리포트가 완료되면 가상 진입·청산 기록을 계산합니다."
...
(() => {
  ...
  async function runAdminAction(action, button) {
    ...
    const actionLabel = isPaper ? "AI 가상매매 기록" : isOutcome ? "사후 기록" : "분석 요청 처리";
