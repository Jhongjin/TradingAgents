...
PLANS: dict[str, Plan] = {
    "free": Plan(
        ...
        name="무료",
        price_krw=0,
        tagline="전날 선별 결과와 공개 성적표",
        features=("전날 선별 결과 전체", "공개 5·20일 성적표", "종목 분석 기본 정보", "분석 요청 하루 3회"),
    ...
    "daily": Plan(
        ...
        name="데일리 패스",
        price_krw=10_000,
        tagline="아침 8시, 오늘의 선별 결과가 도착합니다",
        features=("당일 선별 결과 즉시 열람", "강세·약세·판정·리스크 점검 토론 전문", "분석 요청 하루 20회 · 관심종목 10개", "모의투자 계좌 자동 추적과 세 가지 테마", "광고 없음"),
    ...
    "pro": Plan(
        ...
        name="프로",
        price_krw=30_000,
        tagline="내 규칙으로 종목을 선별합니다",
        features=("데일리 패스 전부", "사용자 지정 대상 종목·리스크 파라미터", "본인 KIS 모의투자 키 연동 (주문 초안은 직접 승인)", "주간 PDF 리포트와 JSON API", "분석 요청 하루 60회"),
...
def gate_harness_payload(payload: Mapping[str, Any] | None, access: PlanAccess, *, now: datetime | None = None) -> dict[str, Any] | None:
    ...
    result["plan_gate"] = {
        ...
        "locked": locked_today,
        "debate_transcript": False,
        "reason": "당일 선별 결과는 데일리 패스부터 바로 열립니다. 무료 플랜은 다음 거래일부터 볼 수 있습니다." if locked_today else "토론 전문은 데일리 패스부터 볼 수 있습니다.",
