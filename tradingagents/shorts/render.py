...
def chrome(canvas: Canvas, board: Storyboard, seconds: float, *, watermark: str) -> None:
    """The two marks that never leave: the margin rail, and where this is from.

    A viewer who cannot see how much is left assumes it is long and scrolls, so
    the rail down the left margin fills as the cut runs. It reads as part of
    the page rather than as a player control, which is the point. The watermark
    carries the address through every frame, including the ones people
    screenshot.
    """
...
def render(
    ...
    *,
    ...
    if audio is not None:
        # a narration track arrives already mixed and levelled, so it is muxed
        # as it is; a bare music file is handed a filter by the caller instead
        command += ["-i", str(audio), "-shortest", "-c:a", "aac", "-b:a", "160k"]
        if audio_filter:
